...
# application
set :application, "Meetup Search"
set :deploy_to, "/var/www/meetup-search"
set :user, "prod"
set :use_sudo, false

# repository
set :local_repository, "."
set :repository, "git@github.com:algolia/meetup-search.git"
set :scm, :git
...
namespace :bluepill do
  ...
  task :quit, :roles => [:cron] do
    run "cd #{current_path} && bundle exec bluepill meetup-search --no-privileged stop"
    run "cd #{current_path} && bundle exec bluepill meetup-search --no-privileged quit"
  end
  ...
  task :status, :roles => [:cron] do
    run "cd #{current_path} && bundle exec bluepill meetup-search --no-privileged status"
  end
end
